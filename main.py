...
async def _dispatch_extract(framework: str, url: str) -> Dict[str, Any]:
...
@mcp.tool()
def parse_and_extract(query: str) -> Dict[str, Any]:
    """
    ...
    if not _has_path_after_prefix(url):
        # 不使用 MCP 的分支：直接返回说明
        return {"message": "链接没有指定组件路径，不执行提取。"}

    # 调度到具体实现（同步包装异步）
    return asyncio.run(_dispatch_extract(framework, url))
